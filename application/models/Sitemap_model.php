...
class Sitemap_model extends CI_Model
{
    ...
    public function get_pages()
    {
        ...
        $this->db->where('languages.id', 1); //13MAR23
    ...
    //get posts sitemap daily
    public function get_posts_sitemap($paramArr)
    {
        $post_type = $paramArr["post_type"];
        $this->db->select('p.id,p.title,p.title_slug,p.created_at,p.keywords,p.category_slug,p.image_big,p.image_default,p.image_slider,p.image_mid,p.image_small,p.image_mime,p.image_storage');
        $this->db->where('p.visibility', 1);
        $this->db->where('p.status', 1);
        $this->db->where('p.lang_id', $this->general_settings->site_lang);
        $this->db->where('p.is_scheduled', 0);

        if(!empty($paramArr["type"]) && $paramArr["type"] == "daily"){
            $this->db->where('p.created_at >=',date("Y-m-d H:i:s",strtotime('-'.$paramArr['hours'].' hours')));
            $this->db->where('p.created_at <=',date("Y-m-d H:i:s"));
        }       

        if(!empty($paramArr["per_page"])){
            $this->db->limit($paramArr["per_page"],$paramArr["offset"]);
        }else{
            $this->db->limit($paramArr["default_limit"]);
        }

        
        $this->db->order_by('p.created_at','DESC');
        if($post_type == "webstory"){
            $query = $this->db->get('posts_webstory p');
        }else{
    ...
    public function get_tags()
    {
        ...
        $this->db->where('posts.visibility', 1);
        $this->db->where('posts.is_scheduled', 0);
        $this->db->where('posts.lang_id', 1); //13MAR23
    ...
    public function update_sitemap_settings()
    {
        ...
        $this->db->where('id', $this->general_settings->site_lang);
    ...
    public function add_post_urls($frequency, $last_modification, $priority, $lastmod_time)
    {
        ...
        if (!empty($posts)) {
            foreach ($posts as $post) {
				...
				//$cat_data = $this->get_category($post->category_id);
				
                if (!empty($base_url)) {
                    $this->sitemap_model->add($base_url . $post->category_slug.'/'.$post->title_slug.'-'.$post->id, $frequency, $last_modification, $priority, $priority_value, $lastmod_time);
                }
            }
        }
    }
