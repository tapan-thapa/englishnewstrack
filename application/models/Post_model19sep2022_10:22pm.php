...
class Post_model extends CI_Model
{
    ...
    public function get_latest_category_posts($lang_id)
    {
        $sql = "SELECT id FROM
                ...
                WHERE posts.is_scheduled = 0 AND posts.visibility = 1 AND posts.status = 1 AND image_url is not null
    ...
    {
        $sql = "SELECT * FROM breaking_news where created_at > DATE_SUB(NOW(), INTERVAL 24 HOUR) ORDER BY created_at DESC LIMIT ?";
        $query = $this->db->query($sql, array($this->breaking_posts_limit));
        return $query->result();
    }
    ...
    {
        $sql = "SELECT COUNT(id) AS count FROM breaking_news where created_at > DATE_SUB(NOW(), INTERVAL 24 HOUR)";
        $query = $this->db->query($sql);
        return $query->row()->count;
    }
