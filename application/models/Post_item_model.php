...
class Post_item_model extends CI_Model
{
    ...
    public function add_post_list_items($post_id, $post_type)
    {
        ...
        $image_storages = $this->input->post('list_item_image_storage', true);
        ...
        if (!empty($titles)) {
            for ($i = 0; $i < count($titles); $i++) {
                $data = array(
                    ...
                    'storage' => !empty($image_storages[$i]) ? $image_storages[$i] : '',
        ...
        if (!empty($post_list_items)) {
            foreach ($post_list_items as $post_list_item) {
                ...
                $data = array(
                    ...
                    'storage' => $this->input->post('list_item_image_storage_' . $post_list_item->id, true),
    ...
    {
        if ($post_type == 'gallery') {
            $sql = "SELECT post_gallery_items.*, posts.created_at FROM post_gallery_items INNER JOIN posts ON post_gallery_items.post_id=posts.id where post_gallery_items.post_id = $post_id";
