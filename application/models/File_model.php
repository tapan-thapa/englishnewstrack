...
class File_model extends CI_Model
{
    ...
    */
    ...
    //upload image
    public function upload_image($image_type="")
    {
        ...
        if (!empty($temp_data)) {
            ...
            } elseif($image_type == "webstory") {
                $data["image_big"] = $this->upload_model->post_slider_webstory_image_upload($temp_path);
                $data["image_default"] = $data["image_big"];
                $data["image_slider"] = $data["image_big"];
                $data["image_mid"] = $data["image_big"];
    ...
    //get images
    public function get_images($limit , $image_type="")
    {
        if ($this->general_settings->file_manager_show_files != 1 && $image_type) {
            $sql = "SELECT * FROM images WHERE (user_id = ? or user_id = ?) AND image_type=? ORDER BY images.id DESC LIMIT ?";
            $query = $this->db->query($sql, array(clean_number($this->auth_user->id), clean_number(-1), $image_type, clean_number($limit)));
        } elseif ($this->general_settings->file_manager_show_files != 1) {
            ...
            $sql = "SELECT * FROM images WHERE image_type=? ORDER BY images.id DESC LIMIT ?";
            $query = $this->db->query($sql, array($image_type, clean_number($limit)));
        }else {
    ...
    //search images
    public function search_images($search,$image_type="")
    {
        $like = '%' . $search . '%';
        if($image_type == "library"){
            $sql = "SELECT * FROM images WHERE images.file_name LIKE ? AND image_type=? ORDER BY images.id DESC";
            $query = $this->db->query($sql, array($like, $image_type));
        }elseif ($this->general_settings->file_manager_show_files != 1 && $image_type) {
            $sql = "SELECT * FROM images WHERE user_id = ? AND images.file_name LIKE ? AND image_type=? ORDER BY images.id DESC";
            $query = $this->db->query($sql, array(clean_number($this->auth_user->id), $like, $image_type));
        } elseif ($this->general_settings->file_manager_show_files != 1) {
    ...
    public function upload_video()
    {
        ...
        if ($this->general_settings->storage == "aws_s3") {
            ...
            if (!empty($input_name) && !empty($path)) {
                $file_name = mediaFileName($_FILES[$input_name]['name']);//remove special char
                $this->aws_model->put_object_direct($path . $file_name, $_FILES[$input_name]['tmp_name']);
                if (!empty($file_name)) {
                    $data = array(
