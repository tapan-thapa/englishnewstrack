...
class Upload_model extends CI_Model
{
    ...
    {
        $new_name = $this->create_directory_by_date('images') . 'image_800x450_' . uniqid() . '.jpg';
    ...
    {
        $new_name = $this->create_directory_by_date('images') . 'image_800x_' . uniqid() . '.jpg';
        $new_path = 'uploads/images/' . $new_name;
        $img = Image::make($path)->orientate();
    ...
    //post slider image upload
    public function post_slider_webstory_image_upload($path)
    {
        $new_name = $this->create_directory_by_date('images') . 'image_350x_' . uniqid() . '.jpg';
        $new_path = 'uploads/images/' . $new_name;
        $img = Image::make($path)->orientate();
    ...
    {
        $new_name = $this->create_directory_by_date('images') . 'image_480x270_' . uniqid() . '.jpg';
        $new_path = 'uploads/images/' . $new_name;
        $img = Image::make($path)->orientate();
        $img->fit(480, 270);
        $img->save(FCPATH . $new_path, $this->img_quality);
    ...
    {
        $new_name = $this->create_directory_by_date('images') . 'image_133x80_' . uniqid() . '.jpg';
        $new_path = 'uploads/images/' . $new_name;
        $img = Image::make($path)->orientate();
        $img->fit(133, 80);
        $img->save(FCPATH . $new_path, $this->img_quality);
    ...
    {
        $new_name = $this->create_directory_by_date('quiz') . 'image_800x_' . uniqid() . '.jpg';
    ...
    //avatar image upload
    public function avatar_upload_new($user_id, $path , $width, $height, $new_path="")
    {
        if(empty($new_path)){
            $new_path = 'uploads/profile/avatar_' . $user_id . '_image_'.$width.'x'.$height.'_' . uniqid() . '.jpg';
        }
        $img = Image::make($path)->orientate();
