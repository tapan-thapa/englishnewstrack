...
class Auth_model extends CI_Model
{
    ...
    public function add_user()
    {
        ...
        $data['vk_url'] = $this->input->post('vk_url', true);
		...
		if (!empty($temp_path)) {
			$data["avatar"] = $this->upload_model->avatar_upload($last_id, $temp_path);
			$this->upload_model->delete_temp_image($temp_path);
			//delete old
            if($this->general_settings->storage == 'aws_s3'){
                $this->load->model("aws_model");
                $this->aws_model->upload_file($data["avatar"]);
            }
		}
    ...
    public function edit_user($id)
    {
        ...
        if (!empty($user)) {
            ...
            $temp_path = $this->upload_model->upload_temp_image('file', 'path');
            if (!empty($temp_path)) {
                $data["avatar"] = $this->upload_model->avatar_upload_new($user->id, $temp_path , 240, 240);
                $this->upload_model->delete_temp_image($temp_path);
                //delete old
