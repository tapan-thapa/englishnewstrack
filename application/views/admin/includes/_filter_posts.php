...
<div class="row table-filter-container">
    <div class="col-sm-12">
        <?php echo form_open($form_action, ['method' => 'GET',"id"=>"filter_frm"]); ?>
        ...
        <?php
        if(!empty($filter_page) && ($filter_page!="webstories")){
        ?>
        <div class="item-table-filter" style="display: none;">
            ...
            <select name="post_type" class="form-control">
                ...
                <?php
                if(!empty($filter_page) && ($filter_page=="scheduled_posts" || $filter_page=="drafts_posts")){
                ?>
                <option value="webstories" <?php echo ($this->input->get('post_type', true) == 'webstories') ? 'selected' : ''; ?>><?php echo "Webstory"; ?></option>
                <?php
                }
                ?>
            </select>
        ...
        <div class="item-table-filter" style="width: 180px;">
            ...
            <div class="input-group date">
                <input type="text" class="form-control" name="sdate" id="filter_start_date" placeholder="Start Published Date" value="<?php echo $this->input->get('sdate', true); ?>">
            </div>
        </div>
        <div class="item-table-filter" style="width: 180px;">
            <label>End Date</label>
            <div class="input-group date">
                <input type="text" class="form-control" name="edate" id="filter_end_date" placeholder="End Published Date" value="<?php echo $this->input->get('edate', true); ?>">
            </div>
        </div>
        <div class="item-table-filter">
        ...
        <div class="item-table-filter md-top-10" style="width: 230px; min-width: 65px;">
            <label style="display: block">&nbsp;</label>
            <button type="submit" class="btn bg-purple"><?php echo trans("filter"); ?></button>
            <button type="button" onclick="window.location.href=$('form#filter_frm').attr('action');" class="btn bg-black"><?php echo "Reset"; ?></button>
            <?php 
            if($filter_page=="webstories" || $filter_page=="posts"){
            ?>
            <button type="submit" class="btn bg-black" value="1" name="csv"> <?php echo "Download"; ?></button>
            <?php
            }
            ?>
        </div>
