...
class Admin_controller extends Admin_Core_Controller
{
    ...
    {
        if ($this->ad_model->update_google_adsense_code($this->general_settings->site_lang)) {
            $this->session->set_flashdata('success', trans("msg_updated"));
    ...
    public function preferences_post()
    {
        ...
        redirect(admin_url() . "preferences?tab=" . clean_str($form));
        exit();
    }
     ...
     */
    public function allowed_file_extensions_post()
    {
        check_permission('settings');
        if ($this->settings_model->update_allowed_file_extensions()) {
            $this->session->set_flashdata('success', trans("msg_updated"));
        } else {
            $this->session->set_flashdata('error', trans("msg_error"));
        }
    ...
    */
    ...
    public function users()
    {
        ...
        $idCats = array_column($data['users'], 'id');
        $post_count = [];
        if(!empty($idCats)){
            $sql = "select count(*) as cnt,user_id from posts where user_id IN (".implode(",",$idCats).") group by user_id";
            $query = $this->db->query($sql);
            $postDetails = $query->result(); 
            if(count($postDetails)){
                foreach ($postDetails as $key => $value) {
                    $post_count[$value->user_id] = $value->cnt;
                }
            }
        }
    ...
    public function add_user_post()
    {
        ...
        $this->form_validation->set_rules('password', trans("password"), 'required|min_length[4]|max_length[200]');
    ...
    public function edit_user($id)
    {
        ...
        if ($data['user']->role == "admin" && $this->auth_user->role != "admin") {
            redirect(admin_url() . "users");
            exit();
        }
        $this->load->view('admin/includes/_header', $data);
