...
class Category_controller extends Admin_Core_Controller
{
    ...
    public function categories($id=0)
    {
        ...
        $select_lang = $this->input->get('select_lang', TRUE);
        $data['title'] = trans("categories");
        $data['categories'] = $this->category_model->getParentCategoriesById($id);
        $data['parentListBar'] = $this->category_model->getParentListBarById($id);
    ...
    public function add_category()
    {
        check_permission('categories');
        $data['title'] = trans("categories");
        $data['parent_categories'] = $this->category_model->categoryTree($this->selected_lang->id,'all');
        //print_r($data);
        $data['panel_settings'] = panel_settings();
        $data['lang_search_column'] = 2;
