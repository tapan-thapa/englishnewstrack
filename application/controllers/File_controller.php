...
class File_controller extends Admin_Core_Controller
{
    ...
    */
    ...
    {
        $image_type = trim($this->input->post('image_type', true));
        $image_type = (isset($image_type) && ($image_type=='webstory' || $image_type=='short_videos' || $image_type=='library'))?$image_type:"";
        $this->file_model->upload_image($image_type);
    }
    ...
    public function get_images()
    {
        $image_type = trim($this->input->post('image_type', true));
        $image_type = (isset($image_type) && ($image_type=='webstory' || $image_type=='short_videos' || $image_type=='library'))?$image_type:"";
        $images = $this->file_model->get_images($this->file_count,$image_type);
    ...
    public function search_image_file()
    {
        ...
        $image_type = trim($this->input->post('image_type', true));
        $image_type = (isset($image_type) && ($image_type=='webstory' || $image_type=='short_videos' || $image_type=='library'))?$image_type:"";
        $images = $this->file_model->search_images($search,$image_type);
        //pr($this->db->queries);
        $this->print_images($images);
    }
