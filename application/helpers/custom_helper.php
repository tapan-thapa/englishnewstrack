...
if (!function_exists('replaceTwitter')) {
function replaceTwitter($body) {
  $count = 0;
  $main = '~https://twitter.(.+)~';
  $output_array = array();
  preg_match_all($main, $body, $output_array);
...
libxml_clear_errors();
    ...
    foreach ($dom->getElementsByTagName('blockquote') as $value) {
        //print_r($value);
        foreach($value->getElementsByTagName('a') as $elem){
          ...
          //      echo $href;
            	...
            	$tweetId = end($tmp);
            	//$tweetId = str_replace("NuclearAttack","1516308097773490178",$tweetId);
                $str = preg_replace('%<blockquote[^>]*?>(.*?)</blockquote>%si', '<amp-twitter width="375"  height="472"  layout="responsive"  data-tweetid="' . $tweetId . '"></amp-twitter>', $body, 1);
                $body = preg_replace('%<script.*?>(.*?)</script>%si', '', $str);
            	 //return $str;
            }
        }
    }
...
if (!function_exists('replaceInstagram')) {
function replaceInstagram($body) {
  $count = 0;
  $main = '~https://www.instagram.(.+)~';
  $output_array = array();
  preg_match_all($main, $body, $output_array);
  if (empty($output_array[0]))
    return $body;
  ...
  foreach ($output_array[0] as $key => $value) {
    $tmpP = explode('https://www.instagram.com/p/', $value);
    $tmpTV = explode('https://www.instagram.com/tv/', $value);
    if (isset($tmpP[1]) && $tmpP[1] != '') {
      $tmp = explode('/?utm_source', $tmpP[1]);
    }
    if (isset($tmpTV[1]) && $tmpTV[1] != '') {
      $tmp = explode('/?utm_source', $tmpTV[1]);
    }
    if (!empty($tmp)) {
      $tmp = explode('/"', $tmp[0]);
      $tmpID = explode('/', $tmp[0]);
      $instaId = trim(end($tmpID));
      if ($instaId != "") {
        $tempArray[] = $instaId;
      }
    }
  ...
  if (!empty($tempArray)) {
    $tempArray = array_values(array_unique($tempArray));
  }
  if (!empty($tempArray)) {
    foreach ($tempArray as $key => $value) {
      $body = preg_replace('%<blockquote[^>]*?>(.*?)</blockquote>%si', '<amp-instagram data-shortcode="' . $value . '" data-captioned width="400" height="400" layout="responsive"></amp-instagram>', $body, 1, $count);
    }
...
if (!function_exists('generate_post_url')) {
    function generate_post_url($post, $base_url = null)
    {
        ...
        if (!empty($post)) {
            ...
            /* Code added on 26feb */
            return $base_url .$post->category_slug."/". $post->title_slug."-".$post->id;
            /* End Code added on 26feb */

            if (!empty($post->post_url) && $ci->general_settings->redirect_rss_posts_to_original == 1) {
            //    return $post->post_url;
            }
			
			$cat_data = get_category_id($post->pri_category_id);
			
			if(!empty($post->pri_category_id)){
				 //return $base_url .$cat_data->name_slug."/". $post->title_slug."-".$post->id.".html";
                 return $base_url .$cat_data->name_slug."/". $post->title_slug."-".$post->id;
			}else{
				if(!empty($post->category_slug)){
					//return $base_url .$post->category_slug."/". $post->title_slug."-".$post->id.".html";
                    return $base_url .$post->category_slug."/". $post->title_slug."-".$post->id;
				}else{
...
if (!function_exists('generate_category_url')) {
    function generate_category_url_new($breadcrumb)
    {
        if (is_array($breadcrumb)) {
            return lang_base_url() . implode('/',$breadcrumb);
        }
        return "#";
    }
}
...
if (!function_exists('all_tags')) {
    ...
    {
		$ci =& get_instance();
		$all_tags = $ci->tag_model->get_tags(10);
...
//generate topic url
if (!function_exists('generate_topic_url')) {
    function generate_topic_url($topic_slug)
    {
        if (!empty($topic_slug)) {
            return lang_base_url() . get_route('topic', true) . $topic_slug;
        }
        return "#";
    }
}
    ...
    function get_post_image($post, $image_size)
    {
        ...
        if (!empty($post)) {
            if (!empty($post->image_url)) {
                ...
                if ($image_size == "photoslider") {
                    $path = $ci->aws_base_url . $post->image_path;
                } elseif ($post->image_storage == "aws_s3") {
...
if (!function_exists('get_user_avatar')) {
    function get_user_avatar($avatar_path)
    {
        ...
        if (!empty($avatar_path)) {
            if($ci->general_settings->storage == 'aws_s3' && !empty($ci->aws_base_url)){
                return $ci->aws_base_url.$avatar_path;
            }elseif (file_exists(FCPATH . $avatar_path)) {
...
if (!function_exists('editFilename')) {
    function editFilename($title)
    {
        $filename = cleanurlTitleSlug($title);
        $filename = str_replace("  "," ",strtolower($filename));
        $filename = str_replace(" ","-",strtolower($filename));
        $filename = remove_special_char_title_slug($filename);
        if(empty($filename)){
            return "news-".time();
        }
        return $filename;
...
if (!function_exists('cleanurlTitleSlug')) {
    function cleanurlTitleSlug($url) {
...
if (!function_exists('remove_special_char_title_slug')) {
    function remove_special_char_title_slug($string) {
...
function mediaFileName($imagename) {

    $file = strtolower($imagename); // Get full image name.
    $filename = pathinfo($file, PATHINFO_FILENAME);  // Get image name.
    $extension = pathinfo($file, PATHINFO_EXTENSION);  // Get image extension.
    $filename = cleanurlTitleSlug($filename);  // Removes special chars.
    $filename = remove_special_char_title_slug($filename);  // Removes special chars.
    $filename = str_replace("-","_",$filename);
    if(strlen ($filename) > 50){
        $filename = substr($filename, 0, 50);
    }
    return $filename = $filename.'_'.time().'.'.$extension;
}
if (!function_exists('live_Link_show')) {
    function live_Link_show($post){
        $max_time = strtotime($post->created_at. ' + 24 hours');
        if($post->post_type == "live_post" && strtotime("now")<=$max_time){
            return '<span class="liveLink">Live |</span> ';
        }
        return "";
    }
}
