...
if (!function_exists('get_posts_by_category_id')) {
    function get_posts_by_category_id($category_id, $categories, $home_limit='')
    {
        if($category_id == 358 || $category_id == 357){
            return get_latest_posts('', $home_limit);
        }elseif($category_id == 366 || $category_id == 488){
            return get_webstory_posts('', $home_limit);
        }
        $ci =& get_instance();
        //pr($ci->latest_category_posts);
        if(is_array($ci->latest_category_posts[$category_id])){
            return $ci->latest_category_posts[$category_id];
        }
        return null;
        ...
        /* if (!empty($ci->latest_category_posts)) {
            ...
            if (!empty($category_tree)) {
                $data = array_filter($ci->latest_category_posts, function ($item) use ($category_tree) {
                    return in_array($item->category_id, $category_tree);
                });
                if($home_limit>0){
                    return array_slice($data, 0, $home_limit, true);
                }else{
                    return $data;
                }
...
if (!function_exists('get_category_id')) {
    function get_category_id($id)
    {
        $ci =& get_instance();
            ...
            $categories = $ci->category_model->get_category($id);
        ...
        if (!empty($category_id)) {
            ...
            if (!empty($subcategories)) {
                foreach ($subcategories as $subcategory) {
                    ...
                    /* New Code */
                    $subcategories2 = get_subcategories($subcategory->id, $categories);
                    if (!empty($subcategories2)) {
                        foreach ($subcategories2 as $subcategory2) {
                            array_push($tree, $subcategory2->id);
                            $subcategories3 = get_subcategories($subcategory2->id, $categories);
                            if (!empty($subcategories3)) {
                                foreach ($subcategories3 as $subcategory3) {
                                    array_push($tree, $subcategory3->id);
                                }
                            }
                        }
                    }
        ...
        if (!empty($category_id)) {
            ...
            if (!empty($category) && $category->parent_id != 0) {
                ...
                if (!empty($parent) && $parent->parent_id != 0) {
                    $parent2 = get_category($parent->parent_id, $categories);
                    if (!empty($parent2) && $parent2->parent_id != 0) {
                        $parent3 = get_category($parent2->parent_id, $categories);
                        if (!empty($parent3)) {
                            array_push($tree, $parent3);
                        }
                    }
                    if (!empty($parent2)) {
                        array_push($tree, $parent2);
                    }
                }
                /* End New Code */
                if (!empty($parent)) {
...
//get latest posts
if (!function_exists('get_webstory_posts')) {
    function get_webstory_posts($lang_id, $limit)
    {
        $ci =& get_instance();
        $key = "webstory_posts_limit" . $limit;
        $latest_posts = get_cached_data($key);
        if (empty($latest_posts)) {
            $latest_posts = $ci->post_model->get_webstory_posts($lang_id, $limit);
            set_cache_data_by_lang($key, $latest_posts, $lang_id);
        }
        return $latest_posts;
    }
...
//get special posts
if (!function_exists('get_special_posts')) {
    function get_special_posts()
    {
        $ci =& get_instance();
        $special_posts = get_cached_data('special_posts');
        if (empty($special_posts)) {
            $special_posts = $ci->post_model->get_special_posts();
            set_cache_data('special_posts', $special_posts);
        }
...
//get featured posts
if (!function_exists('get_trending_posts')) {
    function get_trending_posts()
    {
        $ci =& get_instance();
        $trending_posts = get_cached_data('trending_posts');
        if (empty($trending_posts)) {
            $trending_posts = $ci->post_model->get_trending_posts();
            set_cache_data('trending_posts', $trending_posts);
        }
...
if (!function_exists('get_trending_tags')) {
    function get_trending_tags($id)
    {
        $ci =& get_instance();
        $trending_tags = get_cached_data('trending_tags'.$id);
        if (empty($trending_tags)) {
            $trending_tags = $ci->post_model->get_trending_tags($id);
            set_cache_data('trending_tags'.$id, $trending_tags);
        }
...
//get breaking d news
if (!function_exists('get_breaking_news_latest')) {
    function get_breaking_news_latest()
    {
        $ci =& get_instance();
        $breaking_news = get_cached_data('breaking_news');
        if (empty($breaking_news)) {
            $breaking_news = $ci->post_model->get_breaking_news_latest();
...
//get popular video posts
if (!function_exists('get_popular_video_posts')) {
    function get_popular_video_posts($lang_id)
    {
        $ci =& get_instance();
        $popular_v_posts = array();
        if (!function_exists('panel_settings')) {
            exit();
        }
       
            $popular_v_posts = get_cached_data_by_lang('popular_posts_video', $lang_id);
            if (empty($popular_v_posts)) {
                $popular_v_posts = $ci->post_model->get_popular_videos_post($lang_id);
                set_cache_data_by_lang('popular_posts_video', $popular_v_posts, $lang_id);
            }
        
        return $popular_v_posts;
    }
}
