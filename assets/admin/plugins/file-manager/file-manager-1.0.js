...
$('#file_manager_image').on('show.bs.modal', function (e) {
    ...
    img_post_type = ($(e.relatedTarget).attr('data-post-type'))?$(e.relatedTarget).attr('data-post-type'):'';
    img_post_type = (img_post_type == 'short_videos' || img_post_type=='webstory')?img_post_type:'all';
    refresh_images(img_post_type);
});

//filter post wise
$(document).on('click', '#img_post_type_webstory,#img_post_type_short_videos,#img_post_type_all,#img_post_type_library', function () {
    img_post_type = $(this).data("post-type");
    refresh_images(img_post_type);
});
...
//refresh images
function refresh_images(img_post_type='') {
    var data = {};
    data[csfr_token_name] = $.cookie(csfr_cookie_name);
    data['image_type'] = img_post_type;
    $("span[id^=img_post_type_]").removeClass("text-primary");
    $("#img_post_type_"+img_post_type).addClass("text-primary");
    $.ajax({
        type: "POST",
...
function select_image() {
    ...
    if (image_type == 'additional') {
        ...
        var image = '<div class="list-item-image-container">' +
            ...
            '<a class="btn btn-danger btn-sm btn-delete-selected-file-image btn-delete-selected-list-item-image" data-post-type="' + img_post_type + '" data-image-type="list_item" data-list-item-id="' + data_list_item_id + '" data-is-update="' + data_is_update + '">' +
            '<i class="fa fa-times"></i> ' +
            '</a>' +
            '</div>';
